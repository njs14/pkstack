...
Before reproducing planning inside a PK-Stack skill, select Kiro's native workflow:
...
workflow. PK-Stack therefore does not invoke or emulate Spec, Quick Spec, or Bug Fix, and does not
...
native parallel task execution. Once that native plan is ready, PK-Stack wraps it with the relevant
upstream-derived workflow skills, a narrow feature contract, deeper OKF context when needed, and a
current-session `/verified-goal`. It never creates a second task graph or marks a Kiro task checkbox
as executable proof.
...
uses `verified-goal`, an explicit checkable predicate, and the product's supported wait or
