"""Bounded append-only evidence trails for visible PKStack decisions.
...
from pkstack.models import EvidenceEvent, EvidenceReference, EvidenceVerdict
...
DEFAULT_EVIDENCE_DIRECTORY = Path(".pkstack/state/evidence")
...
def _lock_path(root: Path, evidence_relative: str) -> Path:
    ...
    try:
        return workspace_path(root, Path(".pkstack/state/evidence-locks") / f"{digest}.lock")
    except WorkspacePathError as exc:
        raise EvidenceError(str(exc)) from exc
