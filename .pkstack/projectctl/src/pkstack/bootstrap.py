...
| Reuse project understanding | [Durable knowledge](knowledge/index.md) |
| Capture architecture, decisions, or operations | Update the relevant topic under `knowledge/` |
<!-- project-navigation -->
...
DO      .pkstack/bin/projectctl
  ...
  -> KNOW   Wiki/knowledge/ + bounded Kiro retrieval
```
...
This index belongs to the project after setup. Skills and their bundled resources,
agent instructions, and native `.kiro/specs/` remain in their existing locations.
Link to those artifacts; do not copy or synchronize them into Wiki.

Use ignored `Wiki/work/<task>/` for temporary context and drafts. Ordinary KNOW
retrieval reads `Wiki/knowledge/`, `Wiki/features/`, and native `.kiro/specs/`.
Before retaining a finding, update the
existing topic, distinguish decisions from evidence and unresolved hypotheses,
and mark superseded guidance. See the native `okf` skill for the authoring contract.
"""

KNOWLEDGE_INDEX = """---
okf_version: "0.2"
type: Guide
title: Durable project understanding
description: Topic navigation for definitions, decisions, research, and operational knowledge.
---

# Durable project understanding

Organize documents by feature or subject. Create a topic only when there is useful
knowledge to retain, then add its link here. Consult existing definitions and
decisions before repeating an investigation or interview.

Keep accepted decisions, inspected observations, unverified hypotheses, and open
questions distinguishable. An accepted decision is not proof of implementation.
Link to native specs and executable evidence at their authoritative locations.
Update the relevant topic when understanding changes and identify superseded
guidance; do not retain every draft or duplicate a native task graph.

Working notes belong in `Wiki/work/`, outside this retrieval root. Retaining
knowledge never activates instructions or modifies skills, agents, or steering.

## Topics

Add a relative Markdown link when the first project-owned topic is ready.
"""

PROJECT_OWNED_SEEDS = frozenset({Path("Wiki/index.md"), Path("Wiki/knowledge/index.md")})
...
def _render_wiki_index(root: Path) -> str:
    optional_rows = (
        (
            "Wiki/knowledge/pkstack/native-kiro-composition.md",
            "| Understand the runtime boundary | "
            "[Architecture](knowledge/pkstack/native-kiro-composition.md) |",
        ),
        (
            "Wiki/knowledge/pkstack/native-spec-and-okn.md",
            "| Keep planning and retrieval separate | "
            "[Planning decision](knowledge/pkstack/native-spec-and-okn.md) |",
        ),
        (
            "Wiki/knowledge/pkstack/context-depth.md",
            "| Decide how far to search | "
            "[Context-depth runbook](knowledge/pkstack/context-depth.md) |",
        ),
        (
        ...
        (
            ...
            "| Inspect historical release evidence | "
            "[Release report](../reviews/release-status.md) |",
        ),
    )
    rows = [row for relative, row in optional_rows if (root / relative).is_file()]
    rendered = WIKI_INDEX.replace("<!-- project-navigation -->", "\n".join(rows))
    if _has_legacy_knowledge(root):
        rendered = rendered.replace(
            "[Durable knowledge](knowledge/index.md)", "Existing project-owned Wiki documents"
        ).replace("Wiki/knowledge/", "the existing flat Wiki")
    return rendered
...
def _bootstrap_project_locked(
    ...
    *,
    ...
    if not _has_legacy_knowledge(root):
        generated[Path("Wiki/knowledge/index.md")] = KNOWLEDGE_INDEX.encode()
    else:
        notes.append(
            "Existing flat Wiki knowledge is preserved. Classify and migrate it deliberately "
            "before creating Wiki/knowledge/ or using Wiki/work/ Markdown."
        )
    ...
    for relative, content, executable in preview_operations:
        _apply_file(
            ...
            seed_only=relative in PROJECT_OWNED_SEEDS,
        )
    ...
    *,
    ...
    if seed_only:
        ...
        if not result.dry_run:
            _atomic_write(destination, content, executable=False)
        return
