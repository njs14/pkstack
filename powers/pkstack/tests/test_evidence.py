...
from pkstack import evidence as evidence_module
from pkstack.evidence import EvidenceError, append_evidence, audit_evidence
...
def test_default_append_is_bounded_canonical_monotonic_and_auditable(tmp_path: Path) -> None:
    ...
    path = tmp_path / ".pkstack/state/evidence/account-lookup/decision-log.jsonl"
    events = [json.loads(line) for line in path.read_text(encoding="utf-8").splitlines()]
    assert first["path"] == ".pkstack/state/evidence/account-lookup/decision-log.jsonl"
...
def test_secret_shaped_input_is_rejected_without_echo_or_file(
    ...
    assert not (tmp_path / ".pkstack/state/evidence/account-lookup/decision-log.jsonl").exists()
...
def test_committed_trail_requires_flag_and_exact_target(tmp_path: Path) -> None:
    ...
    assert not (tmp_path / ".pkstack/state/evidence/account-lookup").exists()
    assert (
        audit_evidence(
            tmp_path,
            "account-lookup",
