...
from pkstack.features import FeatureMapError, find_feature
...
from pkstack.models import CommandSpec, VerificationResult
...
def test_spec_bridge_rejects_non_string_argv_elements(tmp_path: Path) -> None:
    bridge = _write_native_spec(tmp_path, "typed") / "pkstack-verification.json"
...
def test_spec_bridge_rejects_schema_free_legacy_shape(tmp_path: Path) -> None:
    bridge = _write_native_spec(tmp_path, "legacy") / "pkstack-verification.json"
...
def test_spec_binding_prefers_published_feature_and_preserves_both_provenances(
    ...
    assert {Path(item.path).name for item in contract.spec_artifacts} == {
        "requirements.md",
        "design.md",
        "pkstack-verification.json",
    }
    assert all(len(item.sha256) == 64 for item in contract.spec_artifacts)
    assert json.loads(
        (tmp_path / ".kiro/specs/account-lookup/pkstack-verification.json").read_text()
...
def test_spec_binding_requires_complete_native_artifacts_and_exact_source(
    ...
    assert not (directory / "pkstack-verification.json").exists()
...
@pytest.mark.parametrize(
    "artifact",
    ["requirements.md", "design.md", "pkstack-verification.json"],
)
...
def test_spec_goal_discards_result_when_artifact_drifts_during_verification(
    ...
    monkeypatch.setattr("pkstack.goal.run_command", drift_during_verification)
    with pytest.raises(GoalError, match="changed after goal start"):
        verify_goal(tmp_path)
...
def test_goal_state_locking_fails_closed_without_fcntl(
    ...
    monkeypatch.setattr("pkstack.goal.fcntl", None)
...
def test_verifier_result_is_discarded_when_resume_changes_state(
    ...
    monkeypatch.setattr("pkstack.goal.run_command", resume_during_verification)
...
def test_goal_contract_and_store_error_boundaries(tmp_path: Path) -> None:
    ...
    bridge = _write_native_spec(tmp_path, "broken") / "pkstack-verification.json"
    ...
    hostile.mkdir()
    outside.mkdir()
    (hostile / ".pkstack").symlink_to(outside, target_is_directory=True)
