...
from pkstack.knowledge import KnowledgeError, _reported_root_matches, search, status, validate
...
def test_knowledge_falls_back_only_to_narrow_feature_validation(
    ...
    monkeypatch.setattr("pkstack.knowledge.shutil.which", lambda _: None)
...
def test_canonical_okn_validation_composes_with_feature_map_verdict(
    ...
    monkeypatch.setattr("pkstack.knowledge.shutil.which", lambda _: str(okn))
...
def test_openknowledge_binary_name_is_supported(
    ...
    monkeypatch.setattr("pkstack.knowledge.shutil.which", which)
