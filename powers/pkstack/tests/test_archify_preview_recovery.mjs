...
import { startPreview } from '../skills/archify/upstream/bin/preview.mjs';

const here = path.dirname(fileURLToPath(import.meta.url));
const directory = path.resolve(process.argv[2]);
const input = path.join(directory, 'source.json');
const output = path.join(directory, 'preview.html');
const specification = JSON.parse(fs.readFileSync(path.join(here, '../skills/archify/upstream/examples/web-app.architecture.json'), 'utf8'));
