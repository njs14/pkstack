...
from pkstack.models import FeatureEntrypoint, FeatureSpec, FeatureSubFeature
...
def _stage_feature_file(path: Path, text: str) -> Path:
    descriptor, temporary = tempfile.mkstemp(prefix=".pkstack-map-stage-", dir=path.parent)
...
def _restore_feature_file(path: Path, data: bytes, mode: int) -> None:
    descriptor, temporary = tempfile.mkstemp(prefix=".pkstack-map-rollback-", dir=path.parent)
