...
DEFAULT_PROPOSAL = Path(".pkstack-maintenance/proposal.json")
DEFAULT_POWER_ROOT = Path("powers/pkstack")
DEFAULT_SKILL_PARITY = DEFAULT_POWER_ROOT / "metadata/upstream-skill-parity.json"
_ACCEPT_DIRECTORY = Path(".pkstack-maintenance")
_ACCEPT_LOCK = Path(".pkstack/state/upstream-accept.lock")
...
try:
    import fcntl
except ImportError:  # pragma: no cover - PKStack currently targets macOS/Linux
...
def _fetch_json(url: str, token: str | None, timeout_seconds: float) -> Any:
    ...
    headers = {
        ...
        "User-Agent": "PKStack/0.1 upstream-check",
...
def _open_accept_lock_descriptor(root: Path) -> int:
    ...
    try:
        ...
        pkstack_descriptor = open_directory(
            root_descriptor,
            ".pkstack",
            mode=0o755,
            owner_only=False,
        )
        descriptors.append(pkstack_descriptor)
...
def _generated_parity(preview: BootstrapResult) -> dict[str, Any]:
    ...
    differences = {
        category: sorted(path for path in paths if path != ".pkstack/discovery.json")
