...
from pkstack.goal import GoalError, GoalStore
from pkstack.models import DoctorCheck
...
def run_doctor(root: Path) -> dict[str, Any]:
    ...
    try:
        ...
        internal = root / ".pkstack" / "bin" / "projectctl"
    ...
    required_assets = {
        ...
        "reviewer-agent": root / ".kiro" / "agents" / "pkstack-reviewer.json",
        "verifier-agent": root / ".kiro" / "agents" / "pkstack-verifier.json",
        "pkstack-core-steering": root / ".kiro" / "steering" / "pkstack-core.md",
        "pkstack-safety-steering": root / ".kiro" / "steering" / "pkstack-safety.md",
        "pkstack-typescript-steering": root / ".kiro" / "steering" / "pkstack-typescript.md",
        "pkstack-unslop-steering": root / ".kiro" / "steering" / "pkstack-unslop.md",
        "session-hook": root / ".kiro" / "hooks" / "pkstack-session.json",
        "tripwire-hook": root / ".kiro" / "hooks" / "pkstack-tripwire.json",
    }
        ...
        checks.append(
            DoctorCheck(
                ...
                (
                    ...
                    else "Re-run /pkstack-setup; foreign files are preserved."
                ),
            )
        )
...
def _runtime_integrity_check(root: Path, internal: Path) -> DoctorCheck:
    pyproject = root / ".pkstack" / "projectctl" / "pyproject.toml"
    try:
...
def _goal_state_check(root: Path) -> DoctorCheck:
    try:
        ...
        return DoctorCheck(
            ...
            "Inspect .pkstack/state/goal.json; do not discard active evidence automatically.",
        )
    if state is None:
...
def _archify_runtime_check(root: Path) -> DoctorCheck:
    ...
    try:
        ...
        return DoctorCheck(
            ...
            "Re-run /pkstack-setup and restore the reviewed Archify bundle.",
        )
    if missing:
        return DoctorCheck(
            "archify-runtime",
...
def _runtime_state_ignored(root: Path) -> DoctorCheck:
    try:
        ...
        if not git_path.exists():
            ...
            completed = subprocess.run(
                ["git", "check-ignore", "-q", ".pkstack/state/goal.json"],
                cwd=root,
                capture_output=True,
                check=False,
            )
    ...
    return DoctorCheck(
        ...
        ".pkstack/state/ is ignored" if ignored else ".pkstack/state/ is not ignored",
        None if ignored else "Add .pkstack/state/ to .gitignore.",
    )
