...
in this catalog. The twenty-four [curated additions](../docs/curated-skills.md) bring the
Power total to 76. Setup stays Power-local; the other 75 install into the
...
| Handling | Files at the current revision | PKStack treatment |
|---|---:|---|
...
`show-me-your-work/scripts/log.sh`. PKStack preserves their plan-validation,
...
particular, the adapted [`poteto-kiro-mode/references/workflows.md`](../skills/poteto-kiro-mode/references/workflows.md)
...
Likewise, the `pkstack-setup` replacement preserves the source package's post-setup offer of that
...
| `interrogate` | Alias / consolidation | [`interrogate`](../skills/interrogate/SKILL.md) | The upstream-named route preserves adversarial review through `/pkstack-model-council`, which owns the shared advisory policy. |
...
| `poteto-mode` | Alias / consolidation | [`poteto-kiro-mode`](../skills/poteto-kiro-mode/SKILL.md) | Renames the existing entry workflow while preserving Poteto's playbooks, checkpoints, explicit skip decisions, and native Kiro planning handoffs. Its consolidation disposition is unchanged; no old-name alias or second router ships. |
...
| `setup-pstack` | Native Kiro replacement | [`pkstack-setup`](../skills/pkstack-setup/SKILL.md) | Routes the upstream setup skill to the consistent `/pkstack-setup` Kiro command while preserving the setup outcome and optional verification-workflow offer; replaces its Cursor global role-model rule with idempotent Kiro Power bootstrap, ownership receipts, a pinned local controller, and inheritance of the user's selected Kiro model and effort. |
...
`make-bot-ui` is deliberately discoverable here but has no runnable PKStack skill. The upstream
...
Feature records use PKStack's schema-2 executable contract. The bounded initial-map batch accepts
