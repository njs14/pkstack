...
PK-Stack tracks the `skills/` tree from
...
decision. PK-Stack adapts only the durable knowledge-working methodology into native Kiro workflow
...
activated. If backfill ever becomes a runnable PK-Stack capability, its dependency boundary must
...
and canonical `okn` remains PK-Stack's deterministic validation and retrieval runtime.
