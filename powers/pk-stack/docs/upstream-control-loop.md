# Maintaining PK-Stack upstream ports

GitHub Actions checks the pinned upstream sources daily at 13:17 UTC. A change to an
imported subtree can start one bounded Kiro repair and review cycle. Ordinary Kiro
CLI and IDE sessions do not depend on this pipeline.

## What can ship automatically

The updater can adapt existing skill and documentation text within the paths allowed
by [the maintenance policy](../../../.github/pk-stack-maintenance-policy.json). It
...
uv run --project powers/pk-stack --frozen pytest \
  powers/pk-stack/tests/test_upstreams.py \
  powers/pk-stack/tests/test_release_metadata.py -q
```
