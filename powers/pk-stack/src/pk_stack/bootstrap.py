...
from pk_stack.branding import DISPLAY_NAME, DISTRIBUTION_NAME, RECEIPT_MANAGER
...
BOOTSTRAP_SCHEMA = 1
RECEIPT = Path(".pk-stack/bootstrap.json")
DISCOVERY = Path(".pk-stack/discovery.json")
...
GITIGNORE_RESULT_KEY = ".gitignore:pk-stack-runtime-block"
...
if [ "${PYTHONPATH+x}" = x ]; then
  PK_STACK_CALLER_PYTHONPATH_SET=1
  PK_STACK_CALLER_PYTHONPATH=$PYTHONPATH
else
  PK_STACK_CALLER_PYTHONPATH_SET=0
  PK_STACK_CALLER_PYTHONPATH=
fi
export PK_STACK_CALLER_PYTHONPATH_SET PK_STACK_CALLER_PYTHONPATH
...
set -eu
SCRIPT_DIR=$(CDPATH= cd -- "$(dirname -- "$0")" && pwd)
exec "$SCRIPT_DIR/.pk-stack/bin/projectctl" "$@"
"""
...
def _bootstrap_project_locked(
    ...
    *,
    ...
    cache_root = workspace_path(root, Path(".pk-stack"))
    if power_root is not None:
        ...
        source_package = asset_root / "src" / "pk_stack"
    else:
        checkout_root = Path(__file__).resolve().parents[2]
        checkout_package = checkout_root / "src" / "pk_stack"
    ...
    for source in sorted(source_package.glob("*.py")):
        relative = Path(".pk-stack/projectctl/src/pk_stack") / source.name
        operations.append((relative, source.read_bytes(), False))

    asset_sources = (
        (asset_root / "skills", Path(".kiro/skills"), Path(".pk-stack/projectctl/skills")),
        (
            asset_root / "dev.kiro" / "steering",
            Path(".kiro/steering"),
            Path(".pk-stack/projectctl/dev.kiro/steering"),
        ),
    )
    ...
    generated = {
        Path(".pk-stack/projectctl/pyproject.toml"): TARGET_PYPROJECT.encode(),
        Path(".pk-stack/projectctl/uv.lock"): projectctl_lock.read_bytes(),
        Path(".pk-stack/projectctl/templates/projectctl/uv.lock"): projectctl_lock.read_bytes(),
        Path(".pk-stack/projectctl/README.md"): TARGET_README.encode(),
        Path(".pk-stack/bin/projectctl"): INTERNAL_WRAPPER.encode(),
        Path("Wiki/index.md"): _render_wiki_index(root).encode(),
        Path("Wiki/features/README.md"): _render_feature_readme(root).encode(),
    }
    for relative, content in generated.items():
        operations.append((relative, content, relative == Path(".pk-stack/bin/projectctl")))

    if template_root.is_dir():
        for source in sorted(path for path in template_root.rglob("*") if path.is_file()):
            relative = source.relative_to(template_root)
            cached = Path(".pk-stack/projectctl/templates/project") / relative
            executable = os.access(source, os.X_OK)
            operations.extend(
                (
    ...
    if not root_wrapper.exists() or "projectctl" in previous_hashes:
        ...
        notes.append(
            "An existing root projectctl was preserved; use .pk-stack/bin/projectctl or integrate "
...
def _raise_cached_authority() -> None:
    raise ValueError(
        "refusing to use the project-local .pk-stack cache as setup authority; "
