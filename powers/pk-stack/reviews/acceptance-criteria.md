...
| --- | --- | --- | --- |
| FBL-025 | low | Confirmed in a disposable two-project probe: directly executing project A's cached setup shim can dry-run project B successfully because that explicit path is treated as an external reviewed Power root. This is bounded to a user-selected source, does not escape either workspace, performs no write during dry-run, and remains receipt-auditable. The supported upgrade path remains the active Power-local `/setup-pstack`; users should not reuse another project's cache as an upgrade source. | accepted documented residual; non-material and not release-blocking |
