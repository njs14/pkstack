...
registry semantics. When ``okn`` is absent, only PKStack's narrow executable
feature contracts and local Markdown link targets are checked.
"""
...
def _source_protocol_error(source: object, *, revision_sha: str) -> str | None:
    ...
    if PurePosixPath(path).parts[0] == "work":
        return "okn search returned excluded working material"
...
def status(root: Path) -> dict[str, Any]:
    ...
    layout = "durable-directory" if workspace_safe and knowledge_root.is_dir() else "legacy-flat"
    if layout == "legacy-flat":
        knowledge_root = wiki
    legacy_documents: list[str] = []
    working_documents: list[str] = []
    if workspace_safe and wiki.is_dir():
        for path in wiki.rglob("*"):
            ...
            if relative.parts[0] == "work":
                working_documents.append(relative.as_posix())
            elif relative.parts[0] not in {"features", "knowledge"} and relative != Path(
                "index.md"
            ):
                legacy_documents.append(relative.as_posix())
    layout_error = None
    if layout == "durable-directory" and legacy_documents:
        layout_error = (
            "mixed Wiki layouts would hide legacy knowledge; classify and migrate these "
            "documents before using Wiki/knowledge: " + ", ".join(sorted(legacy_documents))
        )
    elif layout == "legacy-flat" and working_documents:
        layout_error = (
            "canonical okn cannot exclude Wiki/work from the legacy flat Wiki; migrate "
            "durable documents to Wiki/knowledge before validating or searching"
        )
...
def validate(
    ...
    *,
    ...
    link_result = validate_local_links(root, Path(str(info["knowledge_root"])))
    if not info["okn_available"]:
        ...
        return {
            "ok": feature_result["ok"] and link_result["ok"],
    ...
    argv = (
        ...
        "--format",
        "json",
        Path(str(info["knowledge_root"])).relative_to(root.resolve()).as_posix(),
    )
    command = CommandSpec(
        argv=argv,
    ...
    if report is not None:
        protocol_error = _validation_protocol_error(
            report,
            expected_root=Path(str(info["knowledge_root"])),
        )
    okn_ok = (
        result.passed
    ...
    return {
        "ok": feature_result["ok"] and link_result["ok"] and okn_ok,
        "mode": "canonical-okn",
...
def _legacy_search(
    ...
    *,
    ...
    if not ok:
        # Failed protocol output is not usable context, including in raw stdout.
        # In particular a misbehaving tool must not leak excluded draft content.
        context = None
        captured["stdout"] = ""
        captured["stderr"] = ""
    return {
