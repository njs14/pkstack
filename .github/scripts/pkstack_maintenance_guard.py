#!/usr/bin/env python3
"""Trusted boundary checks for PKStack's autonomous maintenance workflows.
...
GENERATED_PREFIXES = (".kiro/", ".pkstack/bin/", ".pkstack/projectctl/")
GENERATED_EXACT = {
    ".pkstack/bootstrap.json",
    ".pkstack/discovery.json",
...
GIT_FINALIZER_MARKER = b"pkstack-trusted-git-finalizer-v1\n"
...
def load_policy(path: Path) -> tuple[bytes, dict[str, Any]]:
    ...
    if authority["path"] != ".kiro/agents/pkstack-maintainer.json":
    ...
    if (
        ...
        or pull_request["branch_prefix"] != "pkstack-upstream/"
        or pull_request["title"] != "[pkstack upstream] Reconcile pinned upstream semantics"
    ):
    ...
    if lifecycle != {
        "workflow_name": "PKStack Upstream Candidate Gate",
        "workflow_path": ".github/workflows/pk-stack-upstream-candidate.yml",
        "run_title_prefix": "PKStack candidate gate for source run ",
    ...
    if (
        ...
        or review["agent"] != "pkstack-ci-reviewer"
    ):
...
def _remove_untracked_generated(root: Path, base_sha: str) -> None:
    ...
    for generated_root in roots:
        ...
        for path in sorted(
            ...
            if relative.startswith(".pkstack/state/"):
                continue
...
def validate_ci_agent(agent_path: Path, policy: dict[str, Any]) -> None:
    ...
    if agent.get("name") != "pkstack-maintainer":
        raise GuardError("CI agent name must be pkstack-maintainer")
...
def validate_product_envelope(root: Path) -> None:
    ...
    for path in hooks:
        ...
        if path.name == "pkstack-session.json":
            ...
            if ".pkstack/bin/projectctl goal tripwire --output json" not in command:
...
def validate_control_plan(root: Path, detector_path: Path, plan_path: Path) -> dict[str, Any]:
    from pkstack_update_controller import decide
...
def validate_package(
    ...
    *,
    ...
    expected = {
        ...
        "source_run_id": source_run_id,
        "base_sha": base_sha,
        "policy_path": ".github/pkstack-maintenance-policy.json",
        "policy_sha256": hashlib.sha256(policy_raw).hexdigest(),
    }
    ...
    with tempfile.TemporaryDirectory(prefix="pkstack-index-") as temporary:
...
def _parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--policy",
        type=Path,
        default=Path(".github/pkstack-maintenance-policy.json"),
    )
