...
SCRIPT = Path(__file__).with_name("verify_pk_stack_attempt.sh")
...
class VerifierExecutionTests(unittest.TestCase):
    def execute(self, *, failure: bool = False, cleanup_failure: bool = False):
        ...
        (module / "__main__.py").write_text(
            ...
            if args[0] == "setup":
                ...
                if "--dry-run" not in args:
                    Path(".pk-stack-maintenance/proposal.json").unlink()
                    Path(".pk-stack-maintenance").rmdir()
        ...
        for name in ("git", "uv"):
            ...
            command.chmod(0o700)
        policy_test = project / ".github/scripts/test_pk_stack_maintenance_guard.py"
        policy_test.parent.mkdir(parents=True)
        policy_test.write_text(
            "# Isolated immutable policy-test double.\n", encoding="utf-8"
        )
        (project / "powers/pk-stack").mkdir(parents=True)
        (project / ".pk-stack-maintenance").mkdir()
        (project / ".pk-stack-maintenance/proposal.json").write_text(
            "{}", encoding="utf-8"
        )
        ...
        diagnostic_root = runner / "pk-stack-verification-results"
    ...
    def test_real_shell_success_reaches_complete_and_emits_only_fixed_metadata(self):
        ...
        self.assertFalse((runner / "pk-stack-verify-1.log").exists())
