...
const policy = {
  ...
  pull_request: {
    ...
    branch_prefix: "pkstack-upstream/",
    title: "[pkstack upstream] Reconcile pinned upstream semantics",
  },
  candidate_lifecycle: {
...
function pull(overrides = {}) {
  return {
    ...
    head: {
      ref: `pkstack-upstream/kiro-${SOURCE_RUN_ID}`,
      sha: HEAD,
...
test("resolves policy name, provider, workflow path, run id, and exact branch together", () => {
  ...
  assert.deepEqual(result.source, {
    ...
    workflowName: "PKStack Upstream Maintenance (Kiro)",
    ...
    expectedHeadRef: `pkstack-upstream/kiro-${SOURCE_RUN_ID}`,
  });
});
...
test("matches captured GitHub Actions REST run shapes exactly", () => {
  ...
  // successful maintenance authority. Use their historical workflow name to
  // exercise the terminal-result check without rewriting captured evidence.
  assert.throws(
    () => resolveCandidateForSource({
      pulls: [],
      ...
      policy: {
        ...policy,
        source_workflows: [{
          ...policy.source_workflows[0],
          name: sourceRunFixture.name,
        }],
      },
...
test("candidate gate is triggered only by Kiro maintenance", () => {
  ...
  assert.match(trigger, /- PKStack Upstream Maintenance \(Kiro\)/);
});
...
test("skips only a fresh active candidate run bound to exact bot, base, head, and source", async () => {
  ...
  assert.deepEqual(result, {
    ...
    ageSeconds: 1800,
    prNumber: 7,
    headRef: `pkstack-upstream/kiro-${SOURCE_RUN_ID}`,
