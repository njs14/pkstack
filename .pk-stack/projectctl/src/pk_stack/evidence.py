...
from pk_stack.models import EvidenceEvent, EvidenceReference, EvidenceVerdict
...
DEFAULT_EVIDENCE_DIRECTORY = Path(".pk-stack/state/evidence")
...
def _lock_path(root: Path, evidence_relative: str) -> Path:
    ...
    try:
        return workspace_path(root, Path(".pk-stack/state/evidence-locks") / f"{digest}.lock")
    except WorkspacePathError as exc:
        raise EvidenceError(str(exc)) from exc
