...
from pk_stack.features import validate_feature_map
from pk_stack.goal import GoalError, GoalStore
from pk_stack.models import DoctorCheck
...
def run_doctor(root: Path) -> dict[str, Any]:
    ...
    try:
        internal = workspace_path(root, Path(".pk-stack/bin/projectctl"))
        internal_error = None
    except WorkspacePathError as exc:
        internal = root / ".pk-stack" / "bin" / "projectctl"
    ...
    if internal_error is None and internal.is_file() and os.access(internal, os.X_OK):
        ...
        checks.append(
            DoctorCheck(
                "projectctl",
                "fail",
                internal_error or "executable .pk-stack/bin/projectctl entrypoint is missing",
    ...
    if wrapper.exists() and not (wrapper.is_file() and os.access(wrapper, os.X_OK)):
        checks.append(
            DoctorCheck(
                "root-projectctl",
                "warn",
                "root projectctl is not an executable file; use .pk-stack/bin/projectctl",
            )
        )
...
def _goal_state_check(root: Path) -> DoctorCheck:
    try:
        ...
        return DoctorCheck(
            ...
            "Inspect .pk-stack/state/goal.json; do not discard active evidence automatically.",
        )
    if state is None:
...
def _runtime_state_ignored(root: Path) -> DoctorCheck:
    try:
        ...
        if not git_path.exists():
            ...
            completed = subprocess.run(
                ["git", "check-ignore", "-q", ".pk-stack/state/goal.json"],
                cwd=root,
                capture_output=True,
                check=False,
            )
    ...
    return DoctorCheck(
        ...
        ".pk-stack/state/ is ignored" if ignored else ".pk-stack/state/ is not ignored",
        None if ignored else "Add .pk-stack/state/ to .gitignore.",
    )
