...
_ACCEPT_LOCK = Path(".pstack/state/upstream-accept.lock")
...
def _open_accept_lock_descriptor(root: Path) -> int:
    ...
    try:
        ...
        pstack_descriptor = open_directory(
            root_descriptor,
            ".pstack",
            mode=0o755,
            owner_only=False,
        )
        descriptors.append(pstack_descriptor)
...
def _generated_parity(preview: BootstrapResult) -> dict[str, Any]:
    ...
    differences = {
        category: sorted(path for path in paths if path != ".pstack/discovery.json")
