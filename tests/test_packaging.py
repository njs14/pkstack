...
def test_extracted_power_assets_and_offline_bootstrap_runtime(tmp_path: Path) -> None:
    ...
    # Consumer guidance must remain navigable after extraction without the repository.
    for document in (
        extracted / "README.md",
        ...
        *extracted.glob("assets/*.md"),
        ...
        while tokens:
            ...
            assert resolved.is_relative_to(extracted.resolve()), (document, href)
            assert resolved.exists(), (document, href)
