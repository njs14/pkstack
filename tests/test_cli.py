...
def test_tampered_state_cannot_authorize_cleanup(tmp_path: Path) -> None:
    ...
    path.write_text(path.read_text().replace("pklab-ci-001-results", "foreign-bucket"), encoding="utf-8")
...
def test_state_manifest_requires_exact_canonical_schema(tmp_path: Path) -> None:
    ...
    save_state(tmp_path, state)
    path = tmp_path / ".lab-state" / "run.json"
    path.write_text(path.read_text(encoding="utf-8").replace("}", ',"extra":"x"}'), encoding="utf-8")
...
def test_deploy_create_service_requests_service_tag_propagation(
    ...
    class Sqs:
        def get_queue_url(self, **kwargs): return {"QueueUrl": "http://floci:4566/queue"}

    class Ecs:
        def register_task_definition(self, **kwargs):
            return {"taskDefinition": {"taskDefinitionArn": f"arn:task-definition/{kwargs['family']}:1"}}
        def create_service(self, **kwargs): calls.append(kwargs)

    services = [{"serviceName": state.api_service, "runningCount": 1}, {"serviceName": state.worker_service, "runningCount": 1}]
    monkeypatch.setattr(cli, "ROOT", tmp_path)
...
def test_deploy_repeat_update_requests_service_tag_propagation(
    ...
    class Ecs:
        ...
        def create_service(self, **kwargs):
            raise ClientError({"Error": {"Code": "AlreadyExists", "Message": "already exists"}}, "CreateService")
        def update_service(self, **kwargs): calls.append(kwargs)
...
def test_foreign_outer_resources_fail_closed(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr(
        cli,
        "_inspect_labels",
        lambda kind, name, missing_ok=False: {"pk-stack-lab.managed": "false"}
        if kind == "container"
        else None,
    )
...
def test_outer_inspection_does_not_turn_generic_docker_failure_into_absence(
    ...
    monkeypatch.setattr(cli, "_run", lambda *args, **kwargs: (_ for _ in ()).throw(cli.LabError("daemon unavailable")))
    with pytest.raises(cli.LabError, match="daemon unavailable"):
        cli._inspect_labels("container", "pk-stack-lab-floci", missing_ok=True)


def test_outer_inspection_accepts_only_positive_exact_absence(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr(cli, "_run", lambda *args, **kwargs: "")
...
def test_task_container_evidence_rejects_socket_mount(monkeypatch: pytest.MonkeyPatch) -> None:
    task_id = "c" * 32
    task = {"taskArn": f"arn:aws:ecs:us-east-1:000000000000:task/cluster/{task_id}", "containers": [{"image": "example:1"}]}
    labels = {"floci": "true", "floci_emulator": "floci-aws", "io.floci": "aws", "io.floci.service": "ecs", "io.floci.resource-id": task_id}
    monkeypatch.setattr(cli, "_inspect_labels", lambda *args, **kwargs: labels)
    monkeypatch.setattr(cli, "_run", lambda *args, **kwargs: "example:1\n")
    monkeypatch.setattr(cli, "_docker_json", lambda args, **kwargs: [{"Destination": "/var/run/docker.sock"}] if ".Mounts" in args[4] else {cli.NETWORK: {}})
...
def test_verify_proves_both_containers_before_docker_exec_or_aws_mutation(
    ...
    monkeypatch.setattr(cli, "_task_container_evidence", lambda *args: (_ for _ in ()).throw(cli.LabError("API and worker task containers must have no mounts")))
...
def test_api_http_rejects_unproven_container_without_docker_exec(monkeypatch: pytest.MonkeyPatch) -> None:
    task_id = "e" * 32
    task = {"taskArn": f"arn:aws:ecs:us-east-1:000000000000:task/cluster/{task_id}"}
    monkeypatch.setattr(cli, "_run", lambda *args, **kwargs: (_ for _ in ()).throw(AssertionError("docker exec must not run")))
...
def test_remaining_current_run_task_leak_is_detected(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(cli, "_inspect_labels", lambda *args, **kwargs: {"io.floci.service": "ecs", "io.floci.resource-id": task_id})
    assert cli._remaining_owned_task_leaks([name]) == [name]


def test_local_image_cleanup_removes_only_exact_current_run_tags(monkeypatch: pytest.MonkeyPatch) -> None:
    state = RunState.create("ci-001")
    commands: list[list[str]] = []
    listings = iter([
        f"{state.prefix}-api:0.1\n{state.prefix}-worker:0.1\nforeign:1\n",
        "foreign:1\n",
    ])
    ...
    assert [command[-1] for command in commands if command[:3] == ["docker", "image", "rm"]] == [
        f"{state.prefix}-api:0.1", f"{state.prefix}-worker:0.1"
    ]
...
def test_service_tasks_rejects_task_tag_mismatch(monkeypatch: pytest.MonkeyPatch) -> None:
    state = RunState.create("ci-001")
    tags = [{"key": "pk-stack-lab:managed", "value": "true"}, {"key": "pk-stack-lab:run", "value": state.run_id}]
    task_id = "a" * 32
    class Ecs:
        def describe_services(self, **kwargs): return {"services": [{"tags": tags}]}
        def list_tasks(self, **kwargs): return {"taskArns": [f"arn:aws:ecs:us-east-1:000000000000:task/{state.cluster}/{task_id}"]}
        def describe_tasks(self, **kwargs): return {"tasks": [{"tags": [], "taskArn": f"arn:aws:ecs:us-east-1:000000000000:task/{state.cluster}/{task_id}", "taskDefinitionArn": f"arn:aws:ecs:us-east-1:000000000000:task-definition/{state.api_family}:1"}]}
    monkeypatch.setattr(cli, "client", lambda name: Ecs())
...
@pytest.mark.parametrize(
    "task",
    [
        {"taskArn": "arn:aws:ecs:us-east-1:000000000000:task/foreign/" + "a" * 32, "taskDefinitionArn": "arn:aws:ecs:us-east-1:000000000000:task-definition/pklab-ci-001-api:1"},
        {"taskArn": "arn:aws:ecs:us-east-1:000000000000:task/pklab-ci-001-cluster/" + "a" * 32, "taskDefinitionArn": "arn:aws:ecs:us-east-1:000000000000:task-definition/foreign:1"},
    ],
)
def test_service_tasks_rejects_wrong_cluster_or_task_definition_family(monkeypatch: pytest.MonkeyPatch, task: dict[str, str]) -> None:
    state = RunState.create("ci-001")
    tags = [{"key": "pk-stack-lab:managed", "value": "true"}, {"key": "pk-stack-lab:run", "value": state.run_id}]
    class Ecs:
        def describe_services(self, **kwargs): return {"services": [{"tags": tags}]}
        def list_tasks(self, **kwargs): return {"taskArns": [task["taskArn"]]}
        def describe_tasks(self, **kwargs): return {"tasks": [{**task, "tags": tags}]}
    monkeypatch.setattr(cli, "client", lambda name: Ecs())
    with pytest.raises(cli.LabError, match="task does not"):
        cli._service_tasks(state)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("ServiceNotFoundException", {"ServiceNotFoundException", "ClusterNotFoundException"}, True),
...
def test_cleanup_missing_codes_are_operation_specific(code: str, allowed: set[str], expected: bool) -> None:
...
def test_up_data_directory_failure_retains_manifest_and_returns_json(
    ...
    monkeypatch.setattr(cli, "_floci_data_path", lambda **kwargs: (_ for _ in ()).throw(OSError("disk error")))
    with pytest.raises(SystemExit) as stopped:
        cli.main(["up", "--run-id", "ci-003", "--output", "json"])
...
def test_down_data_directory_failure_retains_manifest_and_returns_json(
    ...
    monkeypatch.setattr(cli, "_remove_floci_data_path", lambda **kwargs: (_ for _ in ()).throw(OSError("permission error")))
    with pytest.raises(SystemExit) as stopped:
