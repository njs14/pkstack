...
@pytest.mark.parametrize(
    ...
    [
        ...
        [sys.executable, "-qm", "pkstack", "version"],
        ["uv", "run", sys.executable, "-qm", "pkstack", "version"],
        ...
        ["projectctl", "version"],
        ["pkstack", "goal", "status"],
        ["uv", "run", "pkstack", "goal", "status"],
        ["uvx", "--from", "pkstack.whl", "pkstack", "goal", "status"],
        [".pkstack/bin/projectctl", "version"],
        [sys.executable, "-m", "pkstack"],
        [sys.executable, "--check-hash-based-pycs", "default", "-m", "pkstack.cli"],
        ["uv", "run", "true"],
        ["uv", "run", "--quiet", "echo", "proof"],
        ["uv", "run", "-m", "pkstack"],
        ["uv", "run", "-m", "-q", "pkstack", "version"],
        ["uv", "run", "-qm", "pkstack", "version"],
        ["uv", "run", "-mq", "pkstack", "version"],
        ["uv", "run", "-vm", "pkstack", "version"],
        ...
        ["uv", "run", "-hq", "pytest"],
        ["uv", "-qV"],
        ["sh", ".pkstack/bin/projectctl", "version"],
        ["ksh", ".pkstack/bin/projectctl", "version"],
        ["csh", ".pkstack/bin/projectctl", "version"],
        ["tcsh", ".pkstack/bin/projectctl", "version"],
        ["bash", "--debug", ".pkstack/bin/projectctl", "version"],
        ["bash", "+O", "extglob", ".pkstack/bin/projectctl", "version"],
        ["bash", "+o", "errexit", ".pkstack/bin/projectctl", "version"],
        ["uv", "run", "sh", ".pkstack/bin/projectctl", "version"],
        ["time", "pytest"],
        ["caffeinate", "pytest"],
        ["stdbuf", "pytest"],
        ["script", "pytest"],
        ...
        ["uvx", "--from", "pkstack", "projectctl", "version"],
        ["uv", "run", "uvx", "pytest", "--version"],
        ["pytest", "-o", "addopts=--version"],
        ["uv", "run", "pytest", "--override-ini=addopts=--version"],
        ["python", "-m", "pytest", "-oaddopts=-h"],
        ["python", "-m", "runpy", "pkstack"],
        ["uv", "run", "python", "-m", "runpy", "pkstack.__main__"],
        ["python", "src/pkstack/__main__.py", "version"],
        ["uv", "run", "python", "src/pkstack/__main__.py", "version"],
        ["uv", "run", "src/pkstack/__main__.py", "version"],
        ["python", ".pkstack/projectctl/src/pkstack/__main__.py", "version"],
        ["python", "src/pkstack", "version"],
        ["uv", "run", "python", "src/pkstack", "version"],
        ["python", ".pkstack/projectctl/src/pkstack", "version"],
        ["python", "src/pkstack/__pycache__/__main__.pyc", "version"],
        ["pytest", "@../outside/args.txt"],
        ["pytest", "@args.txt"],
        ["uv", "run", "pytest", "@../outside/args.txt"],
        ["cc", "@args.txt"],
        ["python", "-m", "pytest", "@args.txt"],
        ["coverage", "run", "-m", "pkstack", "version"],
        ["coverage", "run", "src/pkstack/__main__.py", "version"],
        ["python", "-m", "coverage", "run", "-m", "pkstack", "version"],
        ["python", "-m", "pip", "-V"],
...
def test_symlink_alias_cannot_turn_controller_into_evidence(
    ...
    controller = tmp_path / ".pkstack" / "bin" / "projectctl"
...
def test_cancelled_goal_reaps_verifier_group_and_releases_lock(
    ...
    from pkstack.goal import get_goal
    ...
    (tmp_path / "controller.py").write_text(
        "import sys\nfrom pathlib import Path\n"
        "from pkstack.goal import start_goal, verify_goal\n"
...
def test_project_named_pkstack_can_run_an_unrelated_local_verifier(tmp_path: Path) -> None:
    project = tmp_path / "pkstack"
    project.mkdir()
...
def test_verifier_restores_wrapper_caller_pythonpath_and_applies_overrides(
    ...
    probe.write_text(
        ...
        "  'set_marker': os.environ.get('PKSTACK_CALLER_PYTHONPATH_SET'),\n"
        "  'value_marker': os.environ.get('PKSTACK_CALLER_PYTHONPATH'),\n"
        "}))\n",
        encoding="utf-8",
    )
    monkeypatch.setenv("PYTHONPATH", "/controller/src")
    monkeypatch.setenv("PKSTACK_CALLER_PYTHONPATH_SET", caller_set)
    monkeypatch.setenv("PKSTACK_CALLER_PYTHONPATH", caller_value)
