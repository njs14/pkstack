...
def test_readmes_link_to_documentation_and_public_entrypoints() -> None:
    for path in (REPOSITORY_ROOT / "README.md", ROOT / "README.md"):
        ...
        assert "/pkstack-setup" in content and "/poteto-kiro-mode <task>" in content
...
def test_documentation_diagrams_keep_sources_and_resolving_previews() -> None:
    artifacts = REPOSITORY_ROOT / "docs" / "artifacts"
    for name in ("pkstack-architecture", "pkstack-task-workflow", "pkstack-updater-workflow"):
        ...
        _png_dimensions(png)
    references = {
        REPOSITORY_ROOT / "README.md": "docs/artifacts/pkstack-architecture.png",
        REPOSITORY_ROOT / "docs" / "architecture.md": "artifacts/pkstack-architecture.png",
        ROOT
        / "docs"
        / "usage.md": "https://github.com/njs14/pkstack/blob/main/docs/artifacts/pkstack-task-workflow.png",
        REPOSITORY_ROOT
        / "docs"
        / "upstream-control-loop.md": "artifacts/pkstack-updater-workflow.png",
    }
    for path, target in references.items():
        assert target in path.read_text(encoding="utf-8")
        assert target.startswith("https://") or (path.parent / target).is_file()
...
def test_active_user_guidance_does_not_use_legacy_pkstack_names() -> None:
    paths = {
        *(
        ...
        ROOT / "README.md",
        REPOSITORY_ROOT / "docs" / "artifacts" / "pkstack-architecture.html",
        REPOSITORY_ROOT / "docs" / "artifacts" / "pkstack-architecture.json",
        *(ROOT / "docs").rglob("*.md"),
