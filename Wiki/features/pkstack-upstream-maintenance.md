...
type: feature
schema_version: 2
slug: pkstack-upstream-maintenance
title: PKStack upstream maintenance
draft: false
verification:
  command:
...
related:
  ...
  - ../knowledge/pkstack/native-spec-and-okn.md
...
[native Kiro composition boundary](../knowledge/pkstack/native-kiro-composition.md), the
[canonical KNOW decision](../knowledge/pkstack/native-spec-and-okn.md), and the
[task-driven context-depth runbook](../knowledge/pkstack/context-depth.md).
...
In the current Kiro IDE agent panel or CLI v3 session, invoke the PKStack maintenance workflow,
...
Run `.pkstack/bin/projectctl upstream check --manifest maintenance/upstreams.json` with
`--power-root powers/pkstack --output json`. Pass a GitHub token only through the environment when
...
Invoke `/pkstack-maintain` in the selected `pkstack` agent, keep one live coordinator, inspect the
...
- OKF source inventories cover every regular blob even when PKStack adapts only a narrow semantic subset.
...
- Only the GitHub source entries configured in `maintenance/upstreams.json` advance autonomously; Kiro product facts are observations, not another self-updating source.
